...
const Review = () => {
  ...
  const submitReview = async (status) => {
    ...
    try {
      const response = await fetch(`http://trade.delmargroup.id:5000/api/review/submit`, {
        method: 'POST',
        headers: {
  ...
  const fetchReview = async () => {
    try {
      const response = await fetch(`http://trade.delmargroup.id:5000/api/review/detail?name=${encodeURIComponent(currentSheetName)}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        }
  ...
  const exportToExcel = async (selectedMonth, selectedYear) => {
    ...
    try {
      const uri = `http://trade.delmargroup.id:7000/report/generate_bongkar/${selectedMonth}/${selectedYear}`;
      // console.log(uri);
  ...
  const fetchReport = async (selectedMonth, selectedYear) => {
    ...
    try {
      const response = await fetch(`http://trade.delmargroup.id:7000/report/generate_bongkar_html/${selectedMonth}/${selectedYear}`);
